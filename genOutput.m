...
elseif isfield(EstimOpt,'ProjectName') && ~isempty(EstimOpt.ProjectName)
    projectName = char(string(EstimOpt.ProjectName));
    fileBase = [modelName '_results_' projectName];
else
    fileBase = [modelName '_results'];
end
...
function fileBase = safeExcelFileBase(fileBase)
    ...
    % Replace Windows-forbidden filename characters and path separators.
    fileBase = regexprep(fileBase,'[<>:"/\\|?*]+',' - ');
    ...
    % Windows/Excel do not like filenames ending with dots or spaces.
    fileBase = regexprep(fileBase,'[\. ]+$','');
    ...
    % Keep names reasonably short for old Excel/COM path handling.
    maxLen = 150;
